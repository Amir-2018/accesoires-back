...
const router = Router();
const publicClient = { id: true, username: true, email: true, createdAt: true, updatedAt: true } as const;
...
router.get("/", asyncHandler(async (req, res) => {
  ...
  const where = { role: "client" as const, ...(search ? { OR: [{ username: { contains: search, mode: "insensitive" as const } }, { email: { contains: search, mode: "insensitive" as const } }] } : {}) };
...
router.patch("/:id", asyncHandler(async (req, res) => {
  ...
  if (!current || current.role !== "client") throw new HttpError(404, "Client introuvable.");
