...
export async function findSources(question) {
...
function fallback(question, sources) {
    if (sources.length === 0) {
...
async function callProvider(question, context) {
    ...
    if (provider === "n8n") {
        ...
        if (!response.ok)
            throw new Error(`n8n a répondu ${response.status}`);
    ...
    if (!response.ok)
        throw new Error(`Le provider IA a répondu ${response.status}`);
    const data = await response.json();
    return { text: data.choices?.[0]?.message?.content ?? "Réponse indisponible.", confidence: 78, model };
}
export async function generateAiReply(question) {
