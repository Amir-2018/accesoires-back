...
async function ensureCategory(name) {
    if (!(await prisma.category.findUnique({ where: { nom: name } })))
        throw new HttpError(400, "Cette catégorie n’existe pas.");
}
...
router.get("/", asyncHandler(async (req, res) => {
    ...
    const where = { ...(category ? { categorie: category } : {}), ...(search ? { OR: [{ nom: { contains: search, mode: "insensitive" } }, { description: { contains: search, mode: "insensitive" } }, { categorie: { contains: search, mode: "insensitive" } }] } : {}) };
