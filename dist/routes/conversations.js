...
const conversationInclude = {
    messages: { orderBy: { createdAt: "asc" }, include: { revisions: true } },
...
const toDbStatus = (value) => value.replace("-", "_");
const toApi = (conversation) => ({ ...conversation, status: String(conversation.status).replace("_", "-"), priority: String(conversation.priority) });
const orderReferencePattern = /\bCMD-?\d+\b/i;
function orderReply(order) {
    const statusLabels = { "en_attente": "En attente", validee: "Validée", expediee: "Expédiée", livree: "Livrée" };
...
async function audit(conversationId, actorId, action, entity, entityId, before, after) {
    await prisma.auditEvent.create({ data: { conversationId, actorId, action, entity, entityId, before: before, after: after } });
}
async function getConversation(id, userId, role) {
    ...
    if (role === "client" && conversation.customerId !== userId)
...
router.post("/", authenticate, asyncHandler(async (req, res) => {
    ...
    const draft = await prisma.aiDraft.create({ data: { conversationId: conversation.id, generatedText: result.text, confidence: result.confidence, model: result.model, sources: sources } });
...
router.get("/", authenticate, asyncHandler(async (req, res) => {
    ...
    const priority = typeof req.query.priority === "string" ? priorityInput.parse(req.query.priority) : undefined;
...
router.get("/analytics/overview", authenticate, requireRoles("agent", "supervisor"), asyncHandler(async (_req, res) => {
    ...
    const motifLabels = { livraison: "Livraison", retour: "Retours", paiement: "Paiement", commande: "Commande", compte: "Compte", produit: "Produit" };
    function motif(conversation) {
        if (conversation.intent?.trim())
            return conversation.intent.trim();
...
router.post("/:id/messages", authenticate, asyncHandler(async (req, res) => {
    ...
    const conversation = await getConversation(id, req.auth.userId, req.auth.role);
    const user = await prisma.user.findUniqueOrThrow({ where: { id: req.auth.userId }, select: { username: true } });
    const message = await prisma.conversationMessage.create({ data: { conversationId: conversation.id, authorId: req.auth.userId, authorRole: req.auth.role, authorNom: user.username, text: input.text } });
...
router.post("/:id/ai-draft", authenticate, requireRoles("agent", "supervisor"), asyncHandler(async (req, res) => {
    ...
    if (!latest)
        throw new HttpError(400, "Aucune question client à traiter.");
    const { result, sources } = await generateAiReply(latest.text);
    const draft = await prisma.aiDraft.create({ data: { conversationId: id, generatedText: result.text, confidence: result.confidence, model: result.model, sources: sources, createdById: req.auth.userId } });
...
router.patch("/:id/drafts/:draftId", authenticate, requireRoles("agent", "supervisor"), asyncHandler(async (req, res) => {
    ...
    if (!draft)
        throw new HttpError(404, "Brouillon IA introuvable.");
...
router.post("/:id/drafts/:draftId/send", authenticate, requireRoles("agent", "supervisor"), asyncHandler(async (req, res) => {
    ...
    const draftId = routeParam(req.params.draftId, "draftId");
    const draft = await prisma.aiDraft.findFirst({ where: { id: draftId, conversationId: id } });
    if (!draft || draft.status !== "approuvee")
        throw new HttpError(409, "Le brouillon doit être approuvé avant envoi.");
    const user = await prisma.user.findUniqueOrThrow({ where: { id: req.auth.userId }, select: { username: true } });
    const text = draft.editedText ?? draft.generatedText;
    const result = await prisma.$transaction(async (tx) => {
        const message = await tx.conversationMessage.create({ data: { conversationId: id, authorId: req.auth.userId, authorRole: req.auth.role, authorNom: user.username, text } });
...
function searchKnowledgeArticles(articles, query) {
